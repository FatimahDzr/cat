#include "SinteringMobility2.h"
// libMesh includes
#include "libmesh/quadrature.h"

registerMooseObject("CrowApp", SinteringMobility2);

template<>
InputParameters validParams<SinteringMobility2>()
{
...
SinteringMobility2::SinteringMobility2(const InputParameters & parameters) :
    Material(parameters),
...
void
SinteringMobility2::computeProperties()
{
  ...
  const Real surface_energy = _surface_energy/(energy_scale*_length_scale);
  ...
  const Real GBmob0_c = _GBmob0 * _time_scale * energy_scale /_length_scale; // Convert to lengthscale^4/(eV*timescale);
  const Real Vm_c = _Vm;
  ...
  for (_qp = 0; _qp < _qrule->n_points(); ++_qp)
  {
    ...
    if (_GBMobility < 0)
      ...
      GBmob = _GBMobility * _time_scale * energy_scale /_length_scale;
    ...
    _M[_qp] = _D[_qp] * Vm_c * energy_scale * _JtoeV / (_kb * _T[_qp])    ;
    _dMdc[_qp] = _dDdc[_qp] * Vm_c * energy_scale * _JtoeV / (_kb * _T[_qp])  ;// - _D[_qp] * Vm / (d2F*d2F) * d3F * _JtoeV;
