#include "SinteringDiffusion0.h"
// libMesh includes
#include "libmesh/quadrature.h"

registerMooseObject("CrowApp", SinteringDiffusion0);

template<>
InputParameters validParams<SinteringDiffusion0>()
{
...
SinteringDiffusion0::SinteringDiffusion0(const InputParameters & parameters) :
    Material(parameters),
...
void
SinteringDiffusion0::computeProperties()
{
  ...
  const Real GB_energy = _GB_energy * 10e16 * _length_scale*_length_scale;
  const Real surface_energy = _surface_energy* 10e16 * _length_scale*_length_scale;
  ...
  const Real GBmob0_c = _GBmob0 * _time_scale /  (10e16 * (_length_scale*_length_scale*_length_scale*_length_scale)); // Convert to lengthscale^4/(eV*timescale);
  ...
  for (_qp = 0; _qp < _qrule->n_points(); ++_qp)
  {
    ...
    if (_GBMobility < 0)
      ...
      GBmob = _GBMobility * _time_scale / (10e16 * _length_scale * _length_scale * _length_scale * _length_scale);
    ...
    _D[_qp] = (Dvol * phi + Dsurf * c * (1.0-c) + Dgb * SumEtaj);// + _Dvap*(1 - phi) ;
    _dDdc[_qp] = Dvol * dphidc + Dsurf * (1.0 - 2.0 * c);
   ...
   // _L[_qp] = 4.0/3.0 * GBmob / int_width_c;
    _L[_qp] = GBmob * surface_energy / _kappa_op[_qp];
    ...
    _M[_qp] = _D[_qp] * Vm / (_kb * _T[_qp] * 10e16) * _JtoeV ;
    _dMdc[_qp] = _dDdc[_qp] * Vm / (_kb * _T[_qp] * 10e16) * _JtoeV ;// - _D[_qp] * Vm / (d2F*d2F) * d3F * _JtoeV;
